...
//    "  gl_FragColor = vec4(fract(x*8.11+vp.y*11.3), fract(x*8.21+vp.y*11.9), fract(x*8.31+vp.y*12.19), 1.0);\n"
  ...
  static const char * fontFragmentShader =
    ...
    "  if(what == OVAL)\n"
    "  {\n"
//    "    float dFC=distance(t.xy/where.zw, where.xy/where.zw);\n"
    "    if(abs(pow(t.x-where.x,2.0)/pow(where.z,2.0) + pow(t.y-where.y,2.0)/pow(where.w,2.0) - 1.0) <= vLineSize*2.5)\n"
//    "    float dFC=distance(t.xy/where.zw, where.xy);\n"
//    "    if(dFC <= where.w+vLineSize/1.1)\n"
//    "      if(dFC >= where.w-vLineSize/1.1)\n"
    ...
    "  {\n"
    "    if(!draw(OTOP,vec4(0.0,0.5,0.8,0.5))) if(!draw(LINE,vec4(0.8,0.5,-0.8,-1.0))) if(!draw(LINE,vec4(-0.8,-1.0,0.8,-1.0)))discard;\n"
    "  }\n"
    ...
    "  {\n"
    "    if(!draw(ORIG,vec4(-0.9,-0.2,1.7,0.8))) if(!draw(LINE,vec4(-0.8,0.6,-0.8,1.0))) if(!draw(LINE,vec4(-0.8,1.0,0.8,1.0)))discard;\n"
    "  }\n"
