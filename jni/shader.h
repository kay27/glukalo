...
# include "config.h"
  ...
  static const char * birdFragmentShader =
    ...
    "  float distanceFromEye = distance(vec2(vp.x*vMul + vState.x*vRadius, vp.y + vState.y*vRadius), vec2(tc.x*vMul,tc.y));\n"
    ...
    "  else if((distanceFromCenter > 0.9*vRadius) && (vState.z>0.1))\n"
    "  {\n"
    "    gl_FragColor=vec4(1.0, 1.0, 0.0, 1.0)*vState.w + vColor*(1.0-distanceFromCenter/3.0)*(1.0-vState.w);\n"
    "    return;\n"
    "  }\n"
