...
# include "config.h"
  ...
  static const char * birdFragmentShader =
    ...
    "  }\n"
    "  else if((dFC < 0.19*vRadius) && (mod(vState.z,4.0)>0.4))\n"
    "  {\n"
...
//    "    gl_FragColor = vec4(1.0-abs(sin(vp.x+vp.y+vp.x*vp.y)),0.0,fract(vp.x*99.1)*abs(sin(vp.x*999.1)),1.0);\n"
  ...
  static const char * missileVertexShader =
    ...
    "  vp = vPos;\n"
    "  tc = vTC.xy;\n"
    "  if((phase>1.999)&&(phase<2.01)) gl_Position = vec4(vPos.xy*8.0+vOffs.xy, vPos.zw);\n"
  ...
  static const char * missileFragmentShader =
    ...
    "void main()\n"
    "{\n"
//    "  if((phase > 1.999)&&(phase < 2.01)) { if(length(vec2(tc.x*vMul, tc.y))>1.0) {discard; return;} gl_FragColor = vec4(abs(fract(sin(tc.x*tc.y*1131.1-11.1)*12311.11)), 0.0, 0.0, 1.0); return; }\n"
//    "  if((phase > 1.999)&&(phase < 2.01)) { if(length(vec2(tc.x*vMul, tc.y))>1.0) {discard; return;} gl_FragColor = vec4(abs(fract(sin(tc.x*tc.y*131.1-11.1)*11.11)), 0.0, 0.0, 1.0); return; }\n"
