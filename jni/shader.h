...
//    "  gl_FragColor = vec4(fract(x*8.11+vp.y*11.3), fract(x*8.21+vp.y*11.9), fract(x*8.31+vp.y*12.19), 1.0);\n"
  ...
  static const char * fontFragmentShader =
    ...
    "{\n"
//    "  vec4 w = where * vec4(vCharMul, 1.0, vCharMul, 1.0);\n"
    "  vec4 w = where;\n"
    "  vec4 t = tc * vec4(vCharMul, 1.0, vCharMul, 1.0);\n"
    "  if(what == OVAL)\n"
    "  {\n"
//    "    float dFC=distance(where.xy/where.zw, tc.xy/where.zw);\n"
    "    float dFC=distance(t.xy/w.zw, w.xy/w.zw);\n"
    "    if(dFC <= w.w+vLineSize/2.0)\n"
    "      if(dFC >= w.w-vLineSize/2.0)\n"
    "      { gl_FragColor = vec4(1.0,0.0,0.0,1.0); return true; }\n"
    "    return false;\n"
    "  }\n"
    "  else if(what == LINE)\n"
    "  {\n"
    "    float a=abs(distance(w.xy,t.xy));\n"
    "    float b=abs(distance(w.zw,t.xy));\n"
    "    float c=abs(distance(w.xy,w.zw));\n"
...
//    "      float dFC=distance(where.xy, vec2(tc.x*where.z/where.w,tc.y));\n"
    "      float dFC=distance(where.xy, tc.xy);\n"
    ...
    "  {\n"
    "    if(!draw(OVAL,vec4(0.0, 0.0, 0.7, 0.7))) discard;\n"
    "  }\n"
    "  else if(vCharCode==49)\n"
    "  {\n"
    "    if(!draw(LINE,vec4(-0.5, 0.0, 0.6, 1.0))) discard;\n"
    "    if(!draw(LINE,vec4( 0.6, 1.0, 0.5,-1.0))) discard;\n"
    "  }\n"
    "  else discard;\n"
    "}\n"
  ;
